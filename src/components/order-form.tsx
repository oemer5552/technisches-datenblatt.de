...
export function OrderForm({ locale }: { locale: Locale }) {
  ...
  return <form className="order-form" onSubmit={submit} encType="multipart/form-data">
    ...
    <div className="form-head"><div><span>{de ? "Sicheres Auftragsformular" : "Secure order form"}</span><h3>{de ? "Fahrzeugdatenpaket bestellen" : "Order vehicle data package"}</h3></div><strong>{product.price}</strong></div>
    ...
    <div className="guarantee-form-note"><Icon name="shield" size={24}/><div><b>{de ? "In der EU akzeptiert oder Geld zurück" : "Accepted in the EU or your money back"}</b><p>{de ? "Lehnt die zuständige Zulassungsstelle in der Europäischen Union unser unverändertes Datenblatt als technischen Nachweis ab, erstatten wir nach den Garantiebedingungen den vollständigen gezahlten Paketpreis." : "If the competent registration authority in the European Union rejects our unaltered data sheet as technical evidence, we refund the full package price paid under the guarantee terms."}</p><Link href={`/${locale}/garantie`} target="_blank">{de ? "Garantiebedingungen ansehen" : "View guarantee terms"}</Link></div></div>
