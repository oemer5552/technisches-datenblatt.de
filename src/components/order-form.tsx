...
export function OrderForm({ locale }: { locale: Locale }) {
  ...
  return <form className="order-form" onSubmit={submit} encType="multipart/form-data">
    ...
    <div className="guarantee-form-note"><Icon name="shield" size={24}/><div><b>{de ? "Europaweit akzeptiert oder Geld zurück" : "Accepted across Europe or your money back"}</b><p>{de ? "Lehnt die zuständige Zulassungsstelle unser unverändertes Datenblatt als technischen Nachweis ab, erstatten wir nach den Garantiebedingungen den vollständigen gezahlten Paketpreis." : "If the competent registration authority rejects our unaltered data sheet as technical evidence, we refund the full package price paid under the guarantee terms."}</p><Link href={`/${locale}/garantie`} target="_blank">{de ? "Garantiebedingungen ansehen" : "View guarantee terms"}</Link></div></div>
