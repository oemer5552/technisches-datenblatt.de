...
export function OrderForm({ locale, paymentsEnabled }: { locale: Locale; paymentsEnabled: boolean }) {
  const de = locale === "de";
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const statusUrl = `/${locale}/status?id=${order.id}&token=${encodeURIComponent(order.accessToken)}`;
      if (!paymentsEnabled) { window.location.assign(statusUrl); return; }
      ...
      window.location.assign(checkout.checkoutUrl || statusUrl);
  ...
  return <form className="order-form" onSubmit={submit} encType="multipart/form-data">
    ...
    <div className="form-head"><div><span>{de ? "Sicheres Auftragsformular" : "Secure order form"}</span><h3>{paymentsEnabled ? (de ? "Fahrzeugdatenpaket bestellen" : "Order vehicle data package") : (de ? "Testauftrag anlegen" : "Create test order")}</h3></div><strong>{paymentsEnabled ? product.price : "TEST"}</strong></div>
    <fieldset disabled={busy}><legend><span>01</span>{de ? "Kontaktdaten" : "Contact details"}</legend><div className="form-grid"><label><span>{de ? "Vor- und Nachname" : "Full name"} *</span><input name="customerName" autoComplete="name" required minLength={2}/></label><label><span>E-Mail *</span><input name="customerEmail" type="email" autoComplete="email" required/></label><label><span>{de ? "Telefon" : "Phone"}</span><input name="customerPhone" autoComplete="tel"/></label><label><span>{de ? "Unternehmen" : "Company"}</span><input name="company" autoComplete="organization"/></label></div></fieldset>
    <fieldset disabled={busy}><legend><span>02</span>{de ? "Fahrzeug" : "Vehicle"}</legend><div className="form-grid"><label className="wide"><span>{de ? "FIN / Fahrgestellnummer" : "VIN"} *</span><input name="vin" minLength={17} maxLength={20} autoCapitalize="characters" spellCheck={false} placeholder="WVWZZZ1JZ3W386752" required/></label><label><span>{de ? "Erstzulassung" : "First registration"} *</span><input name="firstRegistration" type="date" required/></label><label><span>{de ? "EU-Herkunftsland" : "EU country of origin"} *</span><select name="originCountry" defaultValue="" required><option value="" disabled>{de ? "EU-Land wählen" : "Select EU country"}</option>{EU_COUNTRIES.map((country) => <option key={country.code} value={country.code}>{de ? country.de : country.en}</option>)}</select></label><label className="wide"><span>{de ? "Hinweise" : "Notes"}</span><textarea name="notes" rows={2} maxLength={2000} placeholder={de ? "Import nach Deutschland oder Zulassung im EU-Ausland? Besondere Frist oder Rückfrage?" : "Import into Germany or registration in another EU country? Deadline or special question?"}/></label></div></fieldset>
    <fieldset disabled={busy}><legend><span>03</span>{de ? "Pflichtnachweise" : "Required evidence"}</legend><div className="upload-grid"><label className="upload-card"><Icon name="upload"/><b>{de ? "Ausländischer Fahrzeugschein" : "Foreign registration"} *</b><small>PDF, JPG, PNG</small><input name="foreignRegistrationDocument" type="file" accept="application/pdf,image/jpeg,image/png" required/></label><label className="upload-card"><Icon name="upload"/><b>{de ? "Fahrzeugfoto" : "Vehicle photo"} *</b><small>JPG, PNG</small><input name="vehiclePhoto" type="file" accept="image/jpeg,image/png" required/></label><label className="upload-card"><Icon name="upload"/><b>{de ? "Eingeschlagene FIN" : "Stamped VIN"} *</b><small>JPG, PNG</small><input name="stampedVinPhoto" type="file" accept="image/jpeg,image/png" required/></label></div></fieldset>
    {paymentsEnabled ? <div className="guarantee-form-note"><Icon name="shield" size={24}/><div><b>{de ? "In der EU akzeptiert oder Geld zurück" : "Accepted in the EU or your money back"}</b><Link href={`/${locale}/garantie`} target="_blank">{de ? "Garantiebedingungen ansehen" : "View guarantee terms"}</Link></div></div> : <div className="test-mode-note"><Icon name="check" size={20}/><span><b>{de ? "Testbetrieb ohne Zahlung" : "Test mode without payment"}</b><small>{de ? "Nach dem Speichern öffnet sich direkt dein geschützter Auftragsstatus." : "Your protected order status opens immediately after saving."}</small></span></div>}
    <fieldset className="confirmations" disabled={busy}><legend><span>04</span>{de ? "Erklärungen" : "Confirmations"}</legend><label><input type="checkbox" name="vinConfirmation" value="accepted" required/><span>{de ? "Ich habe die eingeschlagene FIN selbst am Fahrzeug geprüft." : "I personally checked the stamped VIN on the vehicle."} *</span></label><label><input type="checkbox" name="privacy" value="accepted" required/><span>{de ? "Ich habe die" : "I have read the"} <Link href={`/${locale}/datenschutz`} target="_blank">{de ? "Datenschutzhinweise" : "privacy notice"}</Link> {de ? "gelesen." : "."} *</span></label><label><input type="checkbox" name="terms" value="accepted" required/><span>{de ? "Ich akzeptiere die" : "I accept the"} <Link href={`/${locale}/agb`} target="_blank">{de ? "AGB einschließlich Akzeptanzgarantie" : "terms including the acceptance guarantee"}</Link>. *</span></label><label><input type="checkbox" name="earlyPerformance" value="accepted" required/><span>{de ? "Ich verlange ausdrücklich, dass vor Ablauf der Widerrufsfrist mit der Dienstleistung begonnen wird." : "I expressly request performance to begin before the cancellation period ends."} *</span></label><label><input type="checkbox" name="withdrawalAck" value="accepted" required/><span>{de ? "Ich habe die" : "I have read the"} <Link href={`/${locale}/widerruf`} target="_blank">{de ? "Widerrufsbelehrung" : "cancellation policy"}</Link> {de ? "gelesen und weiß, dass mein Widerrufsrecht bei vollständiger Vertragserfüllung erlischt." : "and understand that the right ends once the service is fully performed."} *</span></label></fieldset>
    ...
    <button className="button primary submit" type="submit" disabled={busy}>{busy ? (de ? "Wird sicher übertragen …" : "Uploading securely …") : paymentsEnabled ? (de ? `${product.price} · Auftrag verbindlich starten` : `${product.price} · Place binding order`) : (de ? "Kostenlosen Testauftrag anlegen" : "Create free test order")}<Icon name="arrow"/></button><small className="form-note">{paymentsEnabled ? (de ? `${product.price} einmalig inkl. MwSt. · Zahlungsweg im nächsten Schritt` : `${product.price} one-off incl. VAT · Payment in the next step`) : (de ? "Testbetrieb · keine Zahlung · keine Stripe-Weiterleitung" : "Test mode · no payment · no Stripe redirect")}</small>
  </form>;
}
