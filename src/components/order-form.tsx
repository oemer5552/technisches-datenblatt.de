...
export function OrderForm({ locale }: { locale: Locale }) {
  ...
  return <form className="order-form" onSubmit={submit} encType="multipart/form-data">
    <div className="form-head"><div><span>{de ? "Sicheres Auftragsformular" : "Secure order form"}</span><h3>{de ? "Fahrzeug und Käufer" : "Vehicle and customer"}</h3></div><Icon name="shield" size={32}/></div>
    ...
    <button className="button primary submit" type="submit" disabled={busy}>{busy ? (de ? "Wird sicher übertragen …" : "Uploading securely …") : (de ? "Auftrag verbindlich starten" : "Place binding order")}<Icon name="arrow"/></button><small className="form-note">{de ? "Einmalig 24,99 € inkl. MwSt. · Zahlungsweg im nächsten Schritt" : "One-off €24.99 incl. VAT · Payment in the next step"}</small>
  </form>;
}
