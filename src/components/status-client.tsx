...
type OrderStatus = { id: string; reference: string; status: string; paymentStatus: string; createdAt: string; updatedAt: string; files: Array<{ id: string; kind: keyof typeof DOWNLOAD_RESULT_KINDS; label: string; downloadUrl: string }> };

export function StatusClient({ locale, paymentsEnabled }: { locale: Locale; paymentsEnabled: boolean }) {
  ...
  if (order) {
    const hasAiDraft = order.files.some((file) => String(file.kind).startsWith("aiDraft"));
    return <div className="status-card"><div className="status-head"><div><span>{de ? "Auftragsnummer" : "Order reference"}</span><h2>{order.reference}</h2></div><span className="status-pill">{statusLabels[order.status]?.[locale] || order.status}</span></div><div className="status-progress"><span className="done"/><span className={["in_pruefung","rueckfrage","fertiggestellt"].includes(order.status) ? "done" : ""}/><span className={order.status === "fertiggestellt" ? "done" : ""}/></div><div className="status-meta"><div><small>{paymentsEnabled ? (de ? "Zahlung" : "Payment") : (de ? "Modus" : "Mode")}</small><b>{paymentsEnabled ? (order.paymentStatus === "paid" ? (de ? "Bezahlt" : "Paid") : (de ? "Noch offen" : "Pending")) : (de ? "Kostenloser Test" : "Free test")}</b></div><div><small>{de ? "Eingang" : "Received"}</small><b>{new Date(order.createdAt).toLocaleDateString(locale)}</b></div><div><small>{de ? "Letzte Änderung" : "Last update"}</small><b>{new Date(order.updatedAt).toLocaleString(locale)}</b></div></div>{paymentsEnabled ? order.paymentStatus !== "paid" && <div className="notice neutral">{de ? "Die Zahlung ist noch offen." : "Payment is still pending."}</div> : <div className="notice success">{de ? "Testauftrag erfolgreich ohne Zahlung angelegt." : "Test order created successfully without payment."}</div>}{hasAiDraft && <div className="notice neutral">{de ? "Die KI-Entwürfe wurden aus deinen Nachweisen ausgelesen. Sie sind noch nicht technisch geprüft oder unterschrieben." : "The AI drafts were extracted from your evidence. They have not yet been technically reviewed or signed."}</div>}<div className="result-list"><h3>{de ? "Ergebnisdokumente" : "Result documents"}</h3>{order.files.length ? order.files.map((file) => <a key={file.id} href={file.downloadUrl}><Icon name="file"/><span><b>{file.label}</b><small>{de ? "Geschützt herunterladen" : "Protected download"}</small></span><Icon name="arrow"/></a>) : <p>{de ? "Die KI-Auslesung läuft nach dem Eingang automatisch. Bitte lade diese Statusseite in Kürze erneut." : "AI extraction starts automatically after submission. Please reload this status page shortly."}</p>}</div><button className="button ghost small" onClick={() => { setOrder(null); setId(""); setToken(""); }}>{de ? "Anderen Auftrag prüfen" : "Check another order"}</button></div>;
  }
