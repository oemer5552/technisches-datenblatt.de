...
export default async function OrderDetail({ params }: { params: Promise<{ locale: string; id: string }> }) {
  ...
  return <AdminShell locale={locale} title={order.reference}><Link className="back-link" href={`/${locale}/app`}>← Alle Aufträge</Link><div className="detail-grid"><section className="admin-panel"><h2>Kunde & Fahrzeug</h2><dl><dt>Leistung</dt><dd>{order.service === "data" ? "Digitales Fahrzeugdatenpaket" : "Altauftrag"} · {(order.priceCents / 100).toLocaleString("de-DE", { style: "currency", currency: "EUR" })}</dd><dt>Name</dt><dd>{order.customerName}</dd><dt>E-Mail</dt><dd><a href={`mailto:${order.customerEmail}`}>{order.customerEmail}</a></dd><dt>Telefon</dt><dd>{order.customerPhone || "—"}</dd><dt>Unternehmen</dt><dd>{order.company || "—"}</dd><dt>FIN</dt><dd className="mono">{order.vin}</dd><dt>Fahrzeug</dt><dd>{order.make || "—"} {order.model || ""}</dd><dt>Erstzulassung</dt><dd>{order.firstRegistration || "—"}</dd><dt>Herkunft</dt><dd>{order.originCountry || "—"}</dd><dt>FIN-Fundstelle</dt><dd>{order.vinLocation}</dd><dt>Hinweise</dt><dd>{order.notes || "—"}</dd></dl></section><section className="admin-panel"><h2>Dokumente</h2><ul className="document-list">{files.map((file) => <li key={file.id}><div><b>{labels[file.kind] || file.kind}</b><small>{file.originalName} · {(file.size / 1024 / 1024).toFixed(2)} MB</small></div><a className="button ghost small" href={`/api/app/documents/${file.id}`}>Abrufen</a></li>)}</ul></section></div><AdminActions orderId={id} status={order.status} paymentStatus={order.paymentStatus}/><section className="admin-panel timeline"><h2>Protokoll</h2>{events.map((event) => <div key={event.id}><span>{event.createdAt.toLocaleString("de-DE")}</span><b>{event.action}</b><small>{event.actor}</small></div>)}</section></AdminShell>;
}
