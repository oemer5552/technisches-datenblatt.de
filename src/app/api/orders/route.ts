...
export async function POST(request: Request) {
  ...
  try {
    ...
    await getDb().transaction(async (tx) => {
      await tx.insert(orders).values({ id, reference, accessTokenHash: hashToken(accessToken), locale: input.locale, service: input.service, customerName: input.customerName, customerEmail: input.customerEmail, customerPhone: input.customerPhone || null, company: input.company || null, vin: input.vin, make: input.make || null, model: input.model || null, firstRegistration: input.firstRegistration || null, originCountry: input.originCountry || null, vinLocation: input.vinLocation, notes: input.notes || null, priceCents: product.priceCents, currency: SITE.currency, consentData: { version: "2026-07-14.3", recordedAt: now.toISOString(), service: input.service, priceCents: product.priceCents, acceptanceGuarantee: true, terms: true, privacyNotice: true, earlyPerformance: true, withdrawalAcknowledged: true, vinConfirmed: true } });
