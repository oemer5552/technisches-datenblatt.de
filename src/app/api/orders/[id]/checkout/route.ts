...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    line_items: [{ quantity: 1, price_data: { currency: SITE.currency, unit_amount: SITE.priceCents, product_data: { name: "Technisches Datenblatt – Komplettpaket", description: "Typdaten-Prüfung, technisches Datenblatt und FIN-Erklärung" } } }],
