...
export async function GET(request: Request, { params }: { params: Promise<{ id: string; fileId: string }> }) {
  ...
  const [file] = await getDb().select().from(documents).where(and(eq(documents.id, fileId), eq(documents.orderId, id), inArray(documents.kind, Object.keys(RESULT_KINDS)))).limit(1);
